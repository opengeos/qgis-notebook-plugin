#!/usr/bin/env python3
"""
Package the QGIS Notebook Plugin for upload to the official QGIS plugin repository.

This script creates a zip file with the following characteristics:
- Root folder named 'qgis_notebook'
...
Examples:
    ...
    python package_plugin.py --name qgis_notebook
"""
...
def package_plugin(
    ...
    Args:
        source_dir: Path to the qgis_notebook directory
        output_path: Optional path for the output zip file
        target_name: Name for the root folder in the zip (default: 'qgis_notebook')
    ...
    print("╔══════════════════════════════════════════════════════════╗")
    print("║         QGIS Notebook Plugin Packager                    ║")
    print("╚══════════════════════════════════════════════════════════╝")
    print()
    print(f"Source directory: {source_dir}")
    print(f"Output zip file: {output_path}")
    print(f"Root folder name: {target_name}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Package QGIS Notebook Plugin for repository upload",
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Output path for the zip file (default: qgis_notebook-{version}.zip)",
    )
    parser.add_argument(
        "--source",
        "-s",
        type=Path,
        default=None,
        help="Source directory (default: qgis_notebook in the same folder as this script)",
    )
    parser.add_argument(
        "--name",
        "-n",
        type=str,
        default="qgis_notebook",
        help="Target plugin name for the zip root folder (default: qgis_notebook)",
    )
    parser.add_argument(
        "--no-version",
        action="store_true",
    ...
    source_dir = args.source if args.source else script_dir / "qgis_notebook"
